...
int main() {
  ...
  void AppleDivison ( ll sum1, ll sum2, ll ind, vector<ll> &a, ll &mini ) {
      if ( ind == a.size() ) {
         mini = min( mini, abs(sum1 - sum2));
          return;
      }
      AppleDivison(sum1 + a[ind], sum2, ind+1, a, mini);
      AppleDivison(sum1 , sum2 + a[ind], ind+1, a, mini);
  }
